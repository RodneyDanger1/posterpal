...
- **Still open / by design:** #14 (invalidated-on-190 is intentional — dead token, prevents a retry storm; do not "fix").
- **Fixed in this session (Phase 1, 2026-08-22):** #25 (Photo mode now blocks Send with a "Switch to Carousel" action instead of silently dropping extra files), #15 (`publishCarousel` tracks per-slide failures and returns a "published with N of M slides — K failed to upload: …" warning; all-fail still throws). #24 was verified working as-is (extra-Page policy/cadence failures throw → `Extra Pages failed` warning → the composer keeps the caption).
- **Reclassified to Phase 5 (needs production auth + a real paired device):** #28 — the pairing token is the phone's *parallel* trust path, but server fns only accept a Better Auth session; in dev (`VITE_AUTH_ENABLED=false`) `requireUserId` short-circuits to `dev-user`, so a device-token write path is untestable without production auth. Design note for later: `authMiddleware` should accept a `ppd_…` bearer token and resolve it via `resolveDeviceToken()` to `devices.user_id`.
