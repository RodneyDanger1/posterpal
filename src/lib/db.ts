...
async function createPgliteSql(): Promise<Sql> {
  // Persisted under .posterpal-pglite so drafts, tokens, and Later cards survive
  // preview restarts. Set PGLITE_MEMORY=1 to force a throwaway in-memory DB.
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    if (dataDir) console.info("[db] PGLite persisted at", dataDir);
    return pg;
  })().catch((err) => {
